...
spy_logger = logging.getLogger('testIP.spy')
# logger for request/response (if need).
request_logger = logging.getLogger('testIP.request')
...
def error_response(ex):
    resp = application.response_class(json.dumps(ex.dictify()), mimetype=_MIMETYPE_JSON)
    _update_response_headers(resp, _DEFAULT_RESPONSE_HEADERS)
